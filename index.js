...
// Fungsi button kelap kelip

const tombol = new Map();
const posisitombol = new Map();

function TombolRGB(chatId, messageId, keyboard) {
  posisitombol.set(messageId, keyboard);
  if (tombol.has(messageId)) return;

  const styles = ["primary", "success", "danger"];
  let index = 0;
  const interval = setInterval(async () => {
    try {
      const posisi = posisitombol.get(messageId);
      if (!posisi) return;

      index = (index + 1) % styles.length;

      const RGB = posisi.map(row =>
        row.map(btn => ({
          ...btn,
          style: styles[index]
        }))
      ...
      console.error("Tombol RGB Error:",err)
    }
  }, 2000);
  tombol.set(messageId, interval);
}

function MenuRGB(messageId, keyboard){
  posisitombol.set(messageId, keyboard);
}
...
bot.onText(/\/start/, (msg) => {
  ...
  [
        ...
        [
          { text: "Informasi Update Night Shade",
            url: "https://t.me/NightShadeforeverP", 
            style: 'Success',
            icon_custom_emoji_id: '5839406384243807787' },
          { text: "Room Publick Night Shade",
            url: "https://t.me/roompublinightshade1", 
            style: 'Success',
            icon_custom_emoji_id: '5258513401784573443' }
        ],
        [
...
Tap Buton Bellow To Contine -></pre>`,
    parse_mode: "HTML",
    reply_markup: {
...
bot.on("callback_query", async (query) => {
  try {
    ...
    if (query.data === "trashmenu") {
      caption = `<pre>( 🫀 ) OLAA — ${username} さん、お元気ですか？私は Telegram のボットです。賢く使ってください。無実の人に誤って使わないでください。私のチャンネルで更新情報をお待ちください
┗━━━━━━━━━━━━━━⪼
┏━⪼「 ʏᴏᴜʀ ɪɴғᴏʀᴍᴀᴛɪᴏɴ 」
┃ 𖥂 ɴᴀᴍᴇ : ${username}
┃ 𖥂 ɪᴅ : ${senderId}
┃ 𖥂 ᴘʀᴇᴍɪᴜᴍ : ${premiumStatus}
┃ 𖥂 ᴀᴋᴛɪᴘ : ${runtime}
┗━━━━━━━━━━━━━━⪼
┏━⪼「 ʙᴜɢ ᴍᴇɴᴜ 」
┃𖥂 /hyper
...
// DI ATAS ISI LIST MENU ELU
    ...
    if (query.data === "trashmenu2") {
      caption = `
<pre>( 🫀 ) OLAA — ${username} さん、お元気ですか？私は Telegram のボットです。賢く使ってください。無実の人に誤って使わないでください。私のチャンネルで更新情報をお待ちください
┗━━━━━━━━━━━━━━⪼
┏━⪼「 ʏᴏᴜʀ ɪɴғᴏʀᴍᴀᴛɪᴏɴ 」
┃ 𖥂 ɴᴀᴍᴇ : ${username}
┃ 𖥂 ɪᴅ : ${senderId}
...
┃𖥂 @RenzyyDisiniii [ ᴠᴠɪᴘ ]
┃𖥂 @RiizkyAditya [ ꜱᴇᴄᴜʀɪᴛʏ ]
┃𖥂 ᴍʏ ɢᴏᴅ [ ᴀʟʟᴀʜ ]
┃𖥂 ᴍʏ ᴘᴀʀᴇɴᴛ [ ᴅᴀᴅ & ᴍᴏᴍ ]
┗━━━━━━━━━━━━━━▣</pre>`;
      replyMarkup = { inline_keyboard: [[{ text: "Back", callback_data: "back_to_main" }]] };
...
<pre>( 🫀 ) OLAA — ${username} さん、お元気ですか？私は Telegram のボットです。賢く使ってください。無実の人に誤って使わないでください。私のチャンネルで更新情報をお待ちください
┗━━━━━━━━━━━━━━⪼
┏━⪼「 ɴɪɢʜᴛ ꜱʜᴀᴅᴇ ɪɴғᴏʀᴍᴀᴛɪᴏɴ 」
┃ 𖥂 ᴀᴜᴛʜᴏʀ : @joyyyclod
┃ 𖥂 ʙᴏᴛɴᴀᴍᴇ : ɴɪɢʜᴛ ꜱʜᴀᴅᴇ
┃ 𖥂 ᴠᴇʀsɪᴏɴ : 12.0.0 Auto Update
┃ 𖥂 sᴛᴀᴛᴜs : ᴘʀɪᴠᴀᴛᴇ
┗━━━━━━━━━━━━━━⪼
┏━⪼「 ʜᴀʀɢᴀ ᴊᴏɪɴ 」
┃𖥂 Full Update 
